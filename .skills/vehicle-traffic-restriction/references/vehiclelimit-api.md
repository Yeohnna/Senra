...
| Endpoint | `POST https://app-bkdcegaumsjl-api-pLVzAxRQyMWL-gateway.appmiaoda.com/vehiclelimit/query` | `POST https://app-bkdcegaumsjl-api-DYJwnJVBwb4a-gateway.appmiaoda.com/vehiclelimit/city` |
| Auth Header | `X-Gateway-Authorization: Bearer ${INTEGRATIONS_API_KEY}` | 同左 |
| Content-Type | `application/json;charset=UTF-8` | `application/json;charset=UTF-8` |
| third_part_domain | `app-bkdcegaumsjl-api-pLVzAxRQyMWL-gateway.appmiaoda.com` | 同左 |
...
async function queryVehicleLimit(city: string, date: string): Promise<{
  ...
  const url = new URL(
    "https://app-bkdcegaumsjl-api-pLVzAxRQyMWL-gateway.appmiaoda.com/vehiclelimit/query"
  );
...
async function getVehicleLimitCities(): Promise<Array<{city: string; cityname: string}>> {
  const response = await fetch(
    "https://app-bkdcegaumsjl-api-DYJwnJVBwb4a-gateway.appmiaoda.com/vehiclelimit/city",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json;charset=UTF-8",
